...
def measure(stl, path, first, oriented, shift=None):
    ...
    for i, (z, img) in enumerate(layers_of(path, first)):
        ...
        a = PX * PX
        # PIRMAS sluoksnis buna storesnis: PrusaSlicer profilyje
        # `initial_layer_height = 0.3`, t. y. 6 kartus uz kitus. Skaiciuojant ji
        # kaip 0,05 mm, jo dervos likdavo neuzskaityta - puodeliui 87 mm3 is 238,
        # ir del to atrode, kad mes eikvojam 54 % daugiau, nors is tikruju
        # eikvojam maziau (rasta 08-16, kai profilis parode, kad visas skirtumas
        # susitelkes pirmame milimetre).
        storis = first if (i == 0 and first > LAYER) else LAYER
        r['vol'] += sup.sum() * a * storis
        r['model'] += part.sum() * a * storis
        r['layers'] += 1
        if i == 0:
            r['plate'] = sup.sum() * a
