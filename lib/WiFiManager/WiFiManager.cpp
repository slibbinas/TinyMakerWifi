...
bool WiFiManager::shutdownConfigPortal(){
  ...
  if(configPortalActive){
    //DNS handler
    // TinyMakerWiFi patch: the portal can be flagged active while dnsServer/server
    // are already freed (double shutdown between reset() and configPortalActive=false)
    // or were never allocated (low heap at boot). Unguarded, this null-derefs and
    // crashes at boot - confirmed by 0.18.1 crash telemetry (WiFiManager.cpp:966,
    // LoadProhibited, task loopTask). Guard every deref here.
  ...
  if(dnsServer) dnsServer->stop(); //  free heap ?
  dnsServer.reset();
