...
/** The circles a given height needs: every pillar still below its top, with the
    last 0.6 mm tapering to the tip. */
function pillarDiscs(pillars, z) {
  const out = [];
...
async function findOverhangs(pos, layers, onProgress) {
  ...
  const seg = [];
  const cells = new Map();            // 3 mm cell -> the highest overhang in it
  const cellPx = SUP.gridMm * (W / PLATE.x);
  let islands = 0, firstIsland = 0;
  ...
  for (let i = 0; i < layers; i++) {
    ...
    if (hanging) {
      for (let p = 0; p < N; p++) {
        ...
        const k = cx * 4096 + cy;
        let c = cells.get(k);
        if (!c) { c = { px: 0, py: 0, d: Infinity, layer: -1 }; cells.set(k, c); }
        // Only the highest layer in a cell decides where its pillar stands —
        // that is the overhang it has to reach.
        if (c.layer !== i) { c.d = Infinity; c.layer = i; }
        /* The pillar stands on a real overhang pixel, the one nearest the middle
           of the cell. The average of them would read nicer, but on a ring-shaped
           overhang the average lands in the hole — the pillar would then grow
           beside the thing it is meant to hold (audit, 08-13). */
        const mx = (cx + 0.5) * cellPx, my = (cy + 0.5) * cellPx;
        const d = (x - mx) * (x - mx) + (y - my) * (y - my);
        if (d < c.d) { c.d = d; c.px = x; c.py = y; }
      }
  ...
  return { pillars, islands, firstIsland };
}
...
export async function slice(pos, opts, onProgress) {
  ...
  for (let i = 0; i < layers; i++) {
    ...
    const discs = i < SUP.raftLayers
      ? sup.pillars.map(p => ({ x: p.x, y: p.y,
          // A pillar that ends inside the raft needs no foot — it would just be
          // a Ø2.4 mm pancake holding 0.05 mm of nothing.
          r: p.top > SUP.raftLayers * LAYER_MM ? SUP.raftRMm : SUP.rMm }))
      : pillarDiscs(sup.pillars, z);
  ...
  return { blob: zipStore(files), files, layers, rawMl,
           supports: { pillars: sup.pillars.length, raft: sup.pillars.length > 0,
