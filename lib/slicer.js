...
export const SUP = {
  ...
  reachMm: 1.5,
  gridMm:  4.5,    // one pillar per 4.5 mm of overhang — also caps their number
...
function makeBraces(pillars) {
  ...
  const straightTop = p => Math.max(p.bottom, p.top - SUP.tiltMm);
...
export function danglingBraces(pillars, braces) {
  ...
  const holds = (x, y, z) => pillars.some(p => {
    ...
    return Math.hypot(cx - x, cy - y) <= SUP.rMm + 1e-6 &&
           z >= p.bottom - 1e-6 && z <= Math.max(p.bottom, p.top - SUP.tiltMm) + 1e-6;
  });
...
export function supportMesh(pillars, braces) {
  ...
  /* A ball at every corner. Where a leaning cone meets a straight pillar their
     rings sit in different planes, so the joint shows a gap — PrusaSlicer puts
     a junction sphere in exactly those places (V 08-13: „cone galiukai turi
     tarpą nuo stačios dalies"). */
  /* Sudėta iš dviejų kūgių, o ne iš UV sferos: taip ji naudoja tą patį `tube`,
     kuris jau uždaras, ir nereikia atskirai tvarkyti polių — ties 0,45 mm
     skirtumo vis tiek nesimato. */
  const ball = (x, y, z, r) => {
    tube(x, y, z - r, 0.001, x, y, z, r);
    tube(x, y, z, r, x, y, z + r, 0.001);
  };
  ...
  for (const p of pillars) {
    ...
    if (p.bottom === 0 && p.top > SUP.padMm) {
      ...
      tube(cx, cy, 0, SUP.raftRMm, cx, cy, collar, SUP.raftRMm);
      tube(cx, cy, collar, SUP.raftRMm, cx, cy, SUP.padMm, SUP.rMm);
      ...
      tube(cx, cy, p.bottom, SUP.tipMm, cx, cy, fine, SUP.rMm);
      if (bendAt > fine) tube(cx, cy, fine, SUP.rMm, cx, cy, bendAt, SUP.rMm);
    }
    ...
    tube(cx, cy, bendAt, SUP.rMm, ex, ey, ez, SUP.tipMm);
    /* Ball a touch wider than the pillar: at exactly rMm the two rings only
       just meet and a seam still shows through at the bend (V 08-13, twice). */
    if (bendAt > p.bottom && bendAt < p.top) ball(cx, cy, bendAt, SUP.rMm * 1.25);
  }
  for (const c of braces || [])
 ...
 */
async function findOverhangs(pos, layers, onProgress, withSupports) {
    ...
    if (i > 0) {
      ...
      if (hanging) {
        ...
        for (let s = 0; s < N; s++) {
          ...
          if (all === air) {
            islands++;
            if (!firstIsland) firstIsland = i + 1;
            if (islandPillars.length + total < SUP.maxPillars) {
              const a = pickAxis(s % W, (s / W) | 0, lastSolid[s], null);
              islandPillars.push({ px: s % W, py: (s / W) | 0,
                                   ax: a.ax, ay: a.ay, layer: i, abase: a.abase });
            } else dropped = true;
          }
        }
      }
      /* Then the filter, and it is the important one: a pixel that is new is not
         yet a problem — a problem is a pixel FAR from anything solid below.
         Without it every ledge got its own pillar, however small (V 08-13:
         „prusa prie pagrindo išvis mažų suportų neturi"). */
      if (hanging) {
        distanceToSupport();
        for (let p = 0; p < N; p++)
  ...
  for (const list of [...cells.values(), islandPillars]) {
    for (const c of list) {
      ...
      pillars.push({
        ...
        bottom: c.abase >= 0 ? (c.abase + 0.5) * LAYER_MM : 0
      });
    }
  }
  ...
  if (companions.length) {
    ...
    braces = makeBraces(pillars);                  // now they can be tied
  }
