...
| `/api/resin-profile/rename` | POST | `name=<slug>&to=<slug>[&display=<label>]` — SD-card profiles only (409 for a built-in). The name a human reads lives inside the file, so `display` is what actually changes on screen; without it the stored label is kept. `to` may equal `name` (a label-only change: the slug drops capitals and punctuation). The rewrite goes through a temporary file, so no failure can leave the card without a copy of the profile |
