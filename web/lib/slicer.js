...
/* Try the six axis-aligned orientations and keep the one that fits with the
   lowest height. Shorter prints are faster and need fewer supports, so height
   is the right thing to minimise — and if nothing fits, the caller still gets
   the best attempt to scale from. */
export function autoOrient(pos) {
  ...
  for (const o of options) {
    ...
    const rank = (f.fits ? 0 : 1000) + b.size[2];
    if (!best || rank < best.rank) best = { rank, tr, size: b.size, fit: f };
  }
  return best;
}
