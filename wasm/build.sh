...
link() {
  ...
  em++ -std=c++17 -O2 "$OUT"/obj/*.o "${LIBS[@]}"     -o "$OUT/sla-web.js" -sALLOW_MEMORY_GROWTH=1 -sMODULARIZE=1 -sEXPORT_ES6=0     -sEXPORT_NAME=createSLA -sINVOKE_RUN=0 -sFORCE_FILESYSTEM=1     -sEXPORTED_FUNCTIONS='["_sla_slice","_sla_slice_mesh","_sla_export_sl1","_malloc","_free"]'     -sEXPORTED_RUNTIME_METHODS='["ccall","cwrap","FS","UTF8ToString","HEAPF32"]'     --use-port=boost_headers
